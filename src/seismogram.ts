  ...
  return Promise.all([quakeById(eventid), retrieveStationXML()])
    .then(([quake, staxml]) => {
      ...
      if (quakeInfo!=null) {
        ...
        `for ${quake.time.setZone(EasternTimeZone).toLocaleString(DateTime.DATETIME_MED_WITH_SECONDS)} (${latlonF.format(quake.latitude)}, ${latlonF.format(quake.longitude)}) Mag: ${quake.magnitude.mag} Depth: ${depthF.format(quake.depth/1000)}`;
      }
...
function displayForTime(netCode: string, staCode: string, time: DateTime, duration: Duration) {
  ...
  if (quakeInfo!=null) {
    quakeInfo.textContent =
    `for ${netCode}.${staCode} near ${time.setZone(EasternTimeZone).toLocaleString(DateTime.DATETIME_MED_WITH_SECONDS)}`;
  }
  const orgdisp = document.querySelector(sp.organizeddisplay.ORG_DISPLAY) as sp.organizeddisplay.OrganizedDisplay;
